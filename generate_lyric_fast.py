    ...
    with torch.no_grad():
        while True:
            ...
            if mpu.get_model_parallel_rank() == 0:
                ...
                context_tokens = []
                # 将raw_text文本转成 文本内容<eod>文本内容<eod>文本内容<eod> 格式
                for i in raw_text.split("<eod>"):
                    if i != '':
                        context_tokens.extend(tokenizer.encode(i.strip()))
                        context_tokens.append(tokenizer.encoder["<eod>"])
            ...
            while counter < args.out_seq_length - org_context_length:
                if counter == 0:
                ...
                elif context_length < args.seq_length:
                    logits, past_key_values = model(tokens[:, context_length - 1, None],
                                                    position_ids[:, context_length - 1, None],
                    ...
                    for index, item in enumerate(tokens[0, 1:]):
                        tokens[0, index] = item
                    tokens[0, -1] = pad_id
                    past_key_values = [x[:, :, :, 1:] for x in past_key_values]
                    logits, past_key_values = model(tokens[:, args.seq_length - 2, None],
                                                    position_ids[:, args.seq_length - 2, None],
                                                    attention_mask[:, :, args.seq_length - 2,
                                                    :args.seq_length - 1],
                                                    past_key_values=past_key_values, use_cache=True)
                    logits = logits[:, 0, :]
                    ...
                    #     past_key_values = [x[:, :, :, eod_index + 1:] for x in past_key_values]
                    # logits, past_key_values = model(tokens[:, args.seq_length - 2 - eod_index, None],
                    #                                 position_ids[:, args.seq_length - 2 - eod_index, None],
                    #                                 attention_mask[:, :, args.seq_length - 2 - eod_index,
            ...
            if mpu.get_model_parallel_rank() == 0:
                ...
                trim_decode_tokens_1 = decode_tokens_1.replace("<eod>", "\n")
                decode_tokens_2 = tokenizer.decode(output_tokens_list[args.seq_length:])
                trim_decode_tokens_2 = decode_tokens_2.replace("<eod>", "\n")
                print("\nCPM:",
                      trim_decode_tokens_1 + f"{[args.seq_length]}" + trim_decode_tokens_2 + f"{[context_length]}",
                      flush=True)
                result = "Taken time {:.2f}\n".format(
                    time.time() - start_time) + f"\nContext:\n{raw_text}\n\nCPM:\n{trim_decode_tokens_1}{[args.seq_length]}{trim_decode_tokens_2}{[context_length]}"
